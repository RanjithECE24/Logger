...
TEST(LoggerTest, LogLevelOff) {
  ...
  Logger _logger = Logger::CreateLogger(cCtxId, cCtxDescription, cLogLevel);

  std::initializer_list<LogLevel> _logLevels{
      LogLevel::kFatal, LogLevel::kError, LogLevel::kWarn,
...
TEST(LoggerTest, LogLevelVerbose) {
  ...
  std::initializer_list<LogLevel> _logLevels{LogLevel::kFatal, LogLevel::kError,
...
TEST(LoggerTest, WithLevelFunction) {
  ...
  bool _hasId = _logStreamString.find(cCtxId) != std::string::npos;
  ASSERT_TRUE(_hasId);
