...
namespace ara {
namespace core {
InstanceSpecifier::InstanceSpecifier(std::string metaModelIdentifier)
...
void InstanceSpecifier::Serialize(
  ...
  auto _metaModelIdLength{static_cast<uint32_t>(mMetaModelIdentifier.length())};
  ...
  std::copy(mMetaModelIdentifier.cbegin(), mMetaModelIdentifier.cend(),
            std::back_inserter(serializedObject));
}

Result<InstanceSpecifier>
InstanceSpecifier::Create(std::string metaModelIdentifier) {
