...
public:
  ...
  Optional(T &&value) { mValuePtr = new T{std::move(value)}; }
  ...
  template <typename U = T> constexpr Optional &operator=(const U &value) {
    Reset();
  ...
  template <typename U = T> constexpr Optional &operator=(U &&value) {
    Reset();
  ...
  void
  Swap(Optional &other) noexcept(std::is_nothrow_move_assignable<T>::value) {
  ...
  constexpr bool HasValue() const noexcept { return mValuePtr != nullptr; }
...
/// @returns True if both optional instances contain a value and their values
/// are equal or both do not contain a value, otherwise false
template <typename T>
inline bool operator==(const Optional<T> &lhs,
...
/// @returns False if both optional instances contain a value and their values
/// are equal or both do not contain a value, otherwise true
template <typename T>
inline bool operator!=(const Optional<T> &lhs,
                       const Optional<T> &rhs) noexcept {
  bool _result;
