...
public:
  ...
  static Result
  FromValue(const T &t) noexcept(std::is_nothrow_copy_constructible<T>::value);

  /// @brief Result factory by moving its value
  /// @param t Result value
  /// @returns Created Result from the value
  static Result
  FromValue(T &&t) noexcept(std::is_nothrow_move_constructible<T>::value);

  /// @brief Result factory by copying its error
  /// @param e Result error
  /// @returns Created Result from the error
  static Result
  FromError(const E &e) noexcept(std::is_nothrow_copy_constructible<E>::value);

  /// @brief Result factory by moving its error
  /// @param e Result error
  /// @returns Created Result from the error
  static Result
  FromError(E &&e) noexcept(std::is_nothrow_move_constructible<E>::value);

  Result &operator=(Result const &other) noexcept(
      std::is_nothrow_copy_assignable<T>::value
          &&std::is_nothrow_copy_assignable<E>::value) {
    // Copy the value if it exists and reset the error, otherwise copy the error
    // and reset the value
    if (other.HasValue()) {
      mValue = other.mValue;
      mError.Reset();
  ...
  Result &operator=(Result &&other) noexcept(
      std::is_nothrow_move_assignable<T>::value
          &&std::is_nothrow_move_assignable<E>::value) {
    // Move the value if it exists and reset the error, otherwise move the error
    // and reset the value
    if (other.HasValue()) {
      mValue = std::move(other.mValue);
      mError.Reset();
  ...
  /// @brief Construct a new value from the give argument(s) and assign it to
  /// the instance value
  ...
  /// @brief Construct a new error from the give argument(s) and assign it to
  /// the instance error
  ...
  void
  Swap(Result &other) noexcept(std::is_nothrow_move_assignable<T>::value &&
                                   std::is_nothrow_move_assignable<E>::value) {
  ...
  /// @returns True if the instance has a value and false if it contains an
  /// error
  ...
  /// @throws std::runtime_error Throws if there is no value
  T &&operator*() && { return std::move(mValue).Value(); }
  ...
  /// @returns Movable value
  /// @throws std::runtime_error Throws if there is no value
  T &&Value() && { return std::move(mValue).Value(); }
  ...
  /// @throws std::runtime_error Throws if there is no error
  E &&Error() && { return std::move(mError).Value(); }

  /// @brief Get optional instance value
  /// @returns Optional value
  Optional<T> Ok() const & { return mValue; }

  /// @brief Get optional instance value
  /// @returns Optional value
  Optional<T> Ok() && { return std::move(mValue); }

  /// @brief Get optional instance error
  /// @returns Optional error
  Optional<E> Err() const & { return mError; }

  /// @brief Get optional instance error
  /// @returns Optional error
  Optional<E> Err() && { return std::move(mError); }
  ...
  /// @returns True if the input error equals the instance error, otherwise
  /// false
  ...
  /// @throws std::runtime_error Throws if there is no value
  const T &ValueOrThrow() const &noexcept(false) { return mValue.Value(); }

  /// @brief Get instance possible value or throw an exception
  /// @returns Movable value if exists
  /// @throws std::runtime_error Throws if there is no value
  T &&ValueOrThrow() &&noexcept(false) { return std::move(mValue).Value(); }
  ...
  /// @brief Create a new Result by passing the instance value (if exists) to a
  /// callable
  /// @tparam F Callable type
  /// @param f Callable that takes the instance value and returns a new Result
  /// @returns Callable result if the value exists, otherwise a new Result with
  /// the copied instance error
  template <typename F>
...
/// @returns True if both instances contain equal values or equal errors,
/// otherwise false
template <typename T, typename E>
...
/// @returns False if both instances contain equal values or equal errors,
/// otherwise true
template <typename T, typename E>
inline bool operator!=(const Result<T, E> &lhs, const Result<T, E> &rhs) {
  bool _result;
...
private:
  ...
  bool hasError() const noexcept { return mError.HasValue(); }
...
public:
  ...
  constexpr bool HasValue() const noexcept { return !hasError(); }

  /// @returns True if the instance has no error, otherwise false
  constexpr explicit operator bool() const noexcept { return HasValue(); }

  constexpr void operator*() const noexcept {
    // The function does nothing and it is implemented to help with generic
    // programming.
  }
  ...
  /// @brief The function does nothing
  template <typename U> void ValueOr(U &&defaultValue) const noexcept {
    // The function does nothing and it is implemented to help with generic
    // programming.
  }
  ...
  /// @throws std::runtime_error Throws always due to the fact that the instance
  ...
  /// @returns Callable result if the instance does not contain an error,
  /// otherwise a new Result with the copied instance error
  template <typename F> auto Bind(F &&f) const -> Result<decltype(f()), E> {
    if (hasError()) {
...
template <typename E> Result<void, E> Result<void, E>::FromValue() noexcept {
  Result _result;
  return _result;
}
...
/// @returns True if both instances contain no errors or equal errors, otherwise
/// false
template <typename E>
...
/// @returns False if both instances contain no errors or equal errors,
/// otherwise true
template <typename E>
inline bool operator!=(const Result<void, E> &lhs, const Result<void, E> &rhs) {
  bool _result;
